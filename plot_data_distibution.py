...
ALPHA_BLOOD = 0.2
ALPHA_CIFAR10 = 0.2
ALPHA_FASHION_MNIST = 0.3 # Thêm hằng số cho Fashion-MNIST
NUM_CLIENTS = 50
...
fashion_data = load_fashionmnist() # Tải Fashion-MNIST
...
fashion_clients, _ = non_iid_partition_dirichlet(fashion_data, NUM_CLIENTS, "hetero", alpha=ALPHA_FASHION_MNIST) # Phân chia Fashion-MNIST
...
fashion_class_counts = get_class_counts(fashion_data, fashion_clients) # Đếm lớp cho Fashion-MNIST
...
plt.figure(figsize=(20, 6))
x = np.arange(NUM_CLIENTS)
fashion_colors = plt.cm.tab10(np.linspace(0, 1, 10))
for i in range(NUM_CLIENTS):
    bottom = 0
    for class_idx, count in enumerate(fashion_class_counts[i]):
        if count > 0:
            plt.bar(x[i], count, bottom=bottom, color=fashion_colors[class_idx], width=0.8)
            bottom += count
fashion_legend = [plt.Line2D([0], [0], color=fashion_colors[i], lw=5, label=f'Class {i}') for i in range(10)]
plt.legend(handles=fashion_legend, bbox_to_anchor=(0.9, 1), loc='upper left', fontsize=15)
plt.title("Client-wise Class Distribution - Fashion-MNIST", fontsize=30)
plt.xlabel("Client ID", fontsize=25)
plt.ylabel("Number of Samples", fontsize=25)
plt.xticks(np.arange(1, NUM_CLIENTS, step=2), fontsize=25)
plt.yticks(fontsize=25)
...
# === Statistics for MNIST, Fashion-MNIST, BloodMNIST, OrganAMNIST, and CIFAR-10 ===
...
client_class_counts_fashion = [sum(1 for count in counts if count > 0) for counts in fashion_class_counts] # Đếm lớp cho Fashion-MNIST
...
client_sample_counts_fashion = [len(subset) for subset in fashion_clients] # Đếm mẫu cho Fashion-MNIST
...
min_samples_client_fashion = np.argmin(client_sample_counts_fashion)
max_samples_client_fashion = np.argmax(client_sample_counts_fashion)
...
min_classes_client_fashion = np.argmin(client_class_counts_fashion)
max_classes_client_fashion = np.argmax(client_class_counts_fashion)
...
print("\nFashion-MNIST Statistics:") # Thêm thống kê cho Fashion-MNIST
print(f"Clients with the most classes ({client_class_counts_fashion[max_classes_client_fashion]}): Client {max_classes_client_fashion}")
print(f"Clients with the fewest classes ({client_class_counts_fashion[min_classes_client_fashion]}): Client {min_classes_client_fashion}")
print(f"Client with most samples ({client_sample_counts_fashion[max_samples_client_fashion]}): Client {max_samples_client_fashion}")
print(f"Client with fewest samples ({client_sample_counts_fashion[min_samples_client_fashion]}): Client {min_samples_client_fashion}")
...
print(f"Total samples in Fashion-MNIST: {len(fashion_data)}") # Tổng số mẫu cho Fashion-MNIST
...
print("Class distribution in Fashion-MNIST:", np.bincount(np.array(fashion_data.targets))) # Phân bố lớp cho Fashion-MNIST
...
print(f"\nFashion-MNIST - Avg samples/client: {np.mean(client_sample_counts_fashion):.2f}") # Thống kê trung bình cho Fashion-MNIST
print(f"Fashion-MNIST - Max samples: {np.max(client_sample_counts_fashion)}")
print(f"Fashion-MNIST - Min samples: {np.min(client_sample_counts_fashion)}")
print(f"Fashion-MNIST - Avg classes/client: {np.mean(client_class_counts_fashion):.2f}")
print(f"Fashion-MNIST - Max classes: {np.max(client_class_counts_fashion)}")
print(f"Fashion-MNIST - Min classes: {np.min(client_class_counts_fashion)}")
...
print(f"Gini coefficient for Fashion-MNIST class distribution: {gini_coefficient(np.bincount(np.array(fashion_data.targets))):.4f}") # Hệ số Gini cho Fashion-MNIST
